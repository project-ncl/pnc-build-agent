...
public class HttpClient implements Closeable {
    ...
    public HttpClient(OptionMap options) throws IOException {
        this(
                Xnio.getInstance().createWorker(null, options),
    ...
    public CompletableFuture<Response> invoke(Request request, String data) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Making {} request to the endpoint {}; request data: {}.",
                    request.getMethod(),
                    request.getUri(),
                    data);
        }
    ...
    public CompletableFuture<Response> invoke(
        ...
        if (data != null && logger.isTraceEnabled()) {
            logger.trace(
                    "Making request {} {}; Headers: {} request data: {}.",
                    request.getMethod(),
                    request.getUri(),
                    request.getHeaders(),
    ...
    private void invokeAttempt(
        ...
        Function<ByteBufferPool, LimitingStringReadChannelListener> stringReadChannelListener = (
                bufferPool) -> new LimitingStringReadChannelListener(bufferPool, maxDownloadSize) {
                    @Override
