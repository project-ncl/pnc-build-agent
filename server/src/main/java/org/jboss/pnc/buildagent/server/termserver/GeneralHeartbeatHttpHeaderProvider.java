    ...
    public List<Request.Header> getHeaders() {
        if (ldapClient != null) {
            return Collections.singletonList(
                    new Request.Header(HttpHeaders.AUTHORIZATION_STRING, ldapClient.getBasicAuthHeaderValue()));
        } else if (keycloakClient != null) {
            return Collections.singletonList(
                    new Request.Header(HttpHeaders.AUTHORIZATION_STRING, keycloakClient.getBearerAccessToken()));
        } else {
